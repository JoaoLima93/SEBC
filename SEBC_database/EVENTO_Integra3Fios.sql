...
-- Integração de instalações elétricas com 2 fios.
-- Cliente Bifásico (2 Fases e 3 Fios) e Trifásico (3 Fases e 3 Fios)
   ...
   DO insert into CALC_CONSUMO (ID_CLIENTE, CONSUMO, CUSTO_TARIFA_NORMAL, CUSTO_TARIFA_BRANCA, DATA_CONSUMO) 	
      select 	        
             cc.id as ID_CLIENTE,	        
 		     (sum(l.leitura_a + l.leitura_b)/count(l.leitura_a))                           			   as CONSUMO, 	       
             (sum(l.leitura_a + l.leitura_b)/count(l.leitura_a))*avg(tc.preco_kwh)       			   as CUSTO_TARIFA_NORMAL,
             if(f.dia_util='T',(sum(l.leitura_a + l.leitura_b)/count(l.leitura_a))*avg(tb.preco_kwh)	        
      						  ,(sum(l.leitura_a + l.leitura_b)/count(l.leitura_a))*avg(tbf.preco_kwh)) as CUSTO_TARIFA_BRANCA,
