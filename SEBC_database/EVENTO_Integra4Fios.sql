...
-- Cliente Trifásico (3 Fases e 4 Fios)
   ...
   DO insert into CALC_CONSUMO (ID_CLIENTE, CONSUMO, CUSTO_TARIFA_NORMAL, CUSTO_TARIFA_BRANCA, DATA_CONSUMO) 	
      select 	        
             cc.id as ID_CLIENTE,	        
 		     (sum(l.leitura_a + l.leitura_b + l.leitura_c)/count(l.leitura_a))/60                         			      as CONSUMO, 	       
             ((sum(l.leitura_a + l.leitura_b + l.leitura_c)/count(l.leitura_a))/60)*avg(tc.preco_kwh)       			  as CUSTO_TARIFA_NORMAL,
             if(f.dia_util='T',((sum(l.leitura_a + l.leitura_b + l.leitura_c)/count(l.leitura_a))/60)*avg(tb.preco_kwh)	        
      						  ,((sum(l.leitura_a + l.leitura_b + l.leitura_c)/count(l.leitura_a))/60)*avg(tbf.preco_kwh)) as CUSTO_TARIFA_BRANCA,
