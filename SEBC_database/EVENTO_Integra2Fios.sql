...
-- Cliente Monofásico (1Fases e 2Fios) e Bifásico (2 Fases e 2 Fios)
   ...
   DO insert into CALC_CONSUMO (ID_CLIENTE, CONSUMO, CUSTO_TARIFA_NORMAL, CUSTO_TARIFA_BRANCA, DATA_CONSUMO) 	
      select 	        
             ...
             (sum(l.leitura_a)/count(l.leitura_a))*avg(tc.preco_kwh)                     as CUSTO_TARIFA_NORMAL,
             if(f.dia_util='T',(sum(l.leitura_a)/count(l.leitura_a))*avg(tb.preco_kwh)	        
      						  ,(sum(l.leitura_a)/count(l.leitura_a))*avg(tbf.preco_kwh)) as CUSTO_TARIFA_BRANCA,
